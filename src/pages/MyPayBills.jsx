...
const MyPayBills = () => {
  ...
  //   // ... inside handleDownloadReport after canvas is created
  //   const data = canvas.toDataURL("image/png");

  //   const pdf = new jsPDF("p", "mm", "a4"); // Use 'mm' unit for better control
  ...
  //   pdf.save(`bill_report_${dayjs().format("YYYYMMDD")}.pdf`);
  // };
  ...
  //   if (!element) {
  //     Swal.fire("Error", "Report content not found for download.", "error");
  //     return;
  //   }

  //   try {
  //     // 1. UNCOMMENTED & PROTECTED: Capture the HTML element to a canvas
  //     const canvas = await html2canvas(element, {
  //       // Include options here if needed, e.g., to debug the oklch error:
  //       // logging: true,
  //     });

  //     // 2. GET IMAGE DATA: Convert the canvas to PNG data URL
  //     const data = canvas.toDataURL("image/png");

  //     // 3. CORRECT PDF LOGIC: Create PDF and scale the image correctly
  //     const pdf = new jsPDF("p", "mm", "a4"); // Use 'mm' unit
  //     const imgProps = pdf.getImageProperties(data);
  ...
  //     // Use user email for a more specific file name (optional, but good practice)
  //     const userPrefix = user?.email ? user.email.split("@")[0] : "report";
  //     pdf.save(`${userPrefix}_bill_report_${dayjs().format("YYYYMMDD")}.pdf`);
  ...
  //     Swal.fire(
  //       "Report Error",
  //       "Failed to generate PDF. A style incompatibility might exist (e.g., oklch).",
  //       "error"
  //     );
  //   }
  // };
  ...
  // MyPayBills.jsx (inside the component)

  // --- Handlers ---
  // ❌ The printRef is no longer needed since we are not scanning HTML
  // const printRef = React.useRef(null);
